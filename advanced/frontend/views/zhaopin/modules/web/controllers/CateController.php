 ...
 */
class CateController extends BaseWebController
{
    /**
...
//            var_dump($id);die;
        //执行删除语句
        $res = \Yii::$app->db->createCommand()->delete('job_cate',"job_cate_id = $id")->execute();
        //判断是否删除成功
        if($res) {
            echo "<script>alert('删除成功');location.href='info';</script>";
        }
        else {
            echo "<script>alert('删除失败');location.href='info';</script>";
        }
    ...
    public function actionCate_add(){
            ...
            if($this->isPost()){
                ...
                //判断
                if($res){
                    echo "<script>alert('添加成功');location.href='".Urlservice::buildWebUrl('/cate/info')."'</script>";
                }else{
                    echo "<script>alert('添加失败');location.href='".Urlservice::buildWebUrl('/cate/cate_add')."'</script>";
                };
            }
                //查询
